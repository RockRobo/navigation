...
class AmclNode
{
  ...
  //beam skip related params
    ...
    float calcusin(float& theta);
    float calcucos(float& theta);
};
...
//  ROS_INFO("Global range initialisation done!");
  ...
  auto cell_min_x = static_cast<int>(MAP_GXWX(map_, range_min_x));
  auto cell_max_x = static_cast<int>(MAP_GXWX(map_, range_max_x));
  auto cell_min_y = static_cast<int>(MAP_GYWY(map_, range_min_y));
  auto cell_max_y = static_cast<int>(MAP_GYWY(map_, range_max_y));
  ...
  std::vector<float> offset_xn(point_size),
                     offset_yn(point_size),
                     laser_point_x(point_size),
                     laser_point_y(point_size);
...
#pragma omp parallel for
  for (auto th = -M_PIf32; th < M_PIf32;) {
    ...
    for (auto x = cell_min_x; x <= cell_max_x; x += 2) {
      for (auto i = 0; i < point_size; i++) {
        laser_point_x[i] = offset_xn[i] + static_cast<float>(MAP_WXGX(map_, x));
      }
      ...
      for (auto y = cell_min_y; y <= cell_max_y; y += 2) {
        ...
        for (auto i = 0; i < point_size; i++) {
          laser_point_y[i] = offset_yn[i] + static_cast<float>(MAP_WYGY(map_, y));
        }

        current_score = calcuScore(laser_point_x, laser_point_y);
        if (current_score > best_node.score) {
          updated = true;
          while (lock.test_and_set(std::memory_order_acquire));
          best_node.x = x;
          best_node.y = y;
          best_node.theta = th;
          best_node.score = current_score;
          lock.clear(std::memory_order_release);
        }

        if (current_score > local_score) local_score = current_score;
      }
    }
    th += 3 * (M_PIf32 / 180.0);
  }
...
int
AmclNode::calcuScore(std::vector<float>& x, std::vector<float>& y)
{
  int score = 0;
  for (auto i = 0; i < x.size(); i++) {
    if (isMapObs(static_cast<int>(MAP_GXWX(map_, x[i])), static_cast<int>(MAP_GYWY(map_, y[i])))) {
      score++;
    }
  }
...
AmclNode::isMapObs(int x, int y) {
  ...
  for (int i = -range; i <= range; i++) {
    for (int j = -range; j <= range; j++) {
      if (i == 0 && j == 0) continue;
      if (MAP_VALID(map_, x + i, y + j) && map_->cells[MAP_INDEX(map_, x + i, y + j)].occ_state == 1) return true;
    }
  }
...
float
AmclNode::calcusin(float& theta) {
	if (rr_sin_x.empty())
		return 0.0;

	if (theta < -M_PIf32 || theta > M_PIf32) {
		float multiplier = std::floor(theta / (2*M_PIf32));
		theta = theta - multiplier*2*M_PIf32;
		if (theta < -M_PIf32)
			theta += 2*M_PIf32;
		if (theta > M_PIf32)
			theta -= 2*M_PIf32;
	...
	int idx = 0;
	if (theta >= -M_PIf32 && theta < -M_PIf32 / 2.0) {
		idx = static_cast<int>((theta + M_PIf32) / rr_angle_resolution);
		return -rr_sin_x[idx];
	} else if (theta >= -M_PIf32 / 2.0 && theta < 0.0) {
		idx = static_cast<int>((-theta) / rr_angle_resolution);
		return -rr_sin_x[idx];
	} else if (theta >= 0.0 && theta < M_PIf32 / 2.0) {
		idx = static_cast<int>((theta) / rr_angle_resolution);
		return rr_sin_x[idx];
	} else {
		idx = static_cast<int>((-theta + M_PIf32) / rr_angle_resolution);
...
AmclNode::calcucos(float& theta) {
	...
	int idx = 0;
	if (theta >= -M_PIf32 && theta < -M_PIf32 / 2.0) {
		idx = static_cast<int>((-theta - M_PIf32 / 2.0) / rr_angle_resolution);
		return -rr_sin_x[idx];
	} else if (theta >= -M_PIf32 / 2.0 && theta < 0.0) {
		idx = static_cast<int>((theta + M_PIf32 / 2.0) / rr_angle_resolution);
		return rr_sin_x[idx];
